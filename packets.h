...
/* A generic packet containing only the PDP-11 Control Packet Header */
...
typedef struct PacketGeneric CompletePacket_t; /* Complete Packet structure */
typedef struct PacketGeneric UDPReadyPacket_t; /* UDP Ready Packet structure */

/* Control header initialization */
#define CTRL_HEADER_INITIALIZER(TYPE) {.head={'P','D','P'}, .type=TYPE}
static inline void ctrl_header_init(ControlHeader_t *hdr, PType_t type) { /* Initialize a control header at runtime */
	hdr->head[0] = 'P';
	hdr->head[1] = 'D';
	hdr->head[2] = 'P';
	hdr->type = type;
